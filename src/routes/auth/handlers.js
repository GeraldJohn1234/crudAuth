    ...
    return User.findOne({ username: req.payload.username })
        .then(user => {
            if (req.payload.password != Crypto.decrypt(user.password)) {
                return reply.redirect('/?message=Password is Incorrect. &messageTitle=Failed &alertType=danger');
            }
            req.cookieAuth.set(user);
            return reply.redirect('/home');
        }).catch(() => {
            return reply.redirect('/?message=User not found try again. &messageTitle=Failed &alertType=danger');
        })
}
